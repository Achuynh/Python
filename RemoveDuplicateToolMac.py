...
'''To run this script in python3 (required): cmd + shift + P > 'script options' > command > 'python3' > run '''

'''We need access to files and the ability to work with csv files. Import modules.'''
import csv #module to us get csvs
import codecs #Fixes encoding issue with spreadsheet data
import os
import sys
import datetime

'''Initialize global variables for counting records for tool statistics.'''
master_count = 0
...
'''The following three functions returns false if it there is invalid data.'''
def testSD(sd_code):
    if sd_code == "PR" or sd_code == "VI" or sd_code == "GU": #if the subdivision code is GU, PR, or VI
...
def testUQ(user_query):
    if user_query in (None, ''): #If the user query cell is null/empty
...
def testLatLong(lat, long):
    if lat in (None, '') or long in (None, ''): #If the suggested lat/long is empty
        return False
    else:
        #print('Passed validation.')
        return True

'''Checks if the user query of the current row is already in the output file'''
...
with codecs.open(os.path.expanduser('/Users/primaryuser/Desktop/RemoveDuplicateStaging/master.csv'),  'r', encoding='utf-8-sig') as master_csv, codecs.open(output_filename, 'w', encoding = 'utf-8-sig') as output_csv, codecs.open(duplicate_output_filename, 'w', encoding = 'utf-8-sig') as duplicate_output_csv:  # creates output csv file and Creates output csv file of duplicate ids, Open input file
    ...
    with codecs.open(output_filename, 'r', encoding = 'utf-8-sig') as output_csv_read, codecs.open(sd_output_filename, 'w', encoding = 'utf-8-sig') as sd_output_csv, codecs.open(uq_output_filename, 'w', encoding = 'utf-8-sig') as uq_output_csv, codecs.open(latlong_output_filename, 'w', encoding = 'utf-8-sig') as latlong_output_csv: #open reader for created output file
        ...
        print('Checking for duplicates, removing empty queries, and invalid tickets...')
        for row in master_reader: # For each row in the master list
            master_count += 1
            if row['Id'] not in duplicate_ids and testSD(row['Subdivision Code']) and testUQ(row['User Query']) and testLatLong(row['Suggested Lat'], row['Suggested Long']):
                ...
                output_count += 1
            if row['Id'] in duplicate_ids: #Writes the rows of all ids that are deemed 'duplicates' into a new output file
                duplicate_output_writer.writerow(row)
                duplicate_ids_output_count += 1
            if not testSD(row['Subdivision Code']):
                sd_output_writer.writerow(row)
                invalid_country += 1
            if not testUQ(row['User Query']):
                uq_output_writer.writerow(row)
                empty_query += 1
            if not testLatLong(row['Suggested Lat'], row['Suggested Long']):
...
print('Duplicates removed: ' + str(duplicate_ids_output_count))
#print('Double check ticket count (add all categories. This number should match the master sheet row count): ' + str(output_count + invalid_country + empty_query + empty_coordinates + duplicate_ids_output_count))
